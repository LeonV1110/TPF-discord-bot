...
def getAllWhitelisters():
    with connectDatabase() as connection:
        with connection.cursor() as cursor:
            sql = "SELECT * FROM player INNER JOIN `whitelist` on `player.TPFID` = `whitelist.TPFID`" #TODO may need to change the select *
            cursor.execute(sql)
...
def getAllPlayersOnOrder(OrderID):
    with connectDatabase() as connection:
        with connection.cursor() as cursor:
            sql = "SELECT * FROM `player` INNER JOIN `whitelist` ON `player.TPFID` = `whitelist.TPFID` WHERE `whitelist.orderID` = %s"
...
#input a new whitelist into the database
def inputWhitelist(orderID, TPFID):
    with connectDatabase as connection:
        with connection.cursor() as cursor:
            sql = "INSERT INTO `whitelist` (`orderID`, `TPFID`) VALUES (%s, %s)"
...
def updateTier(orderID, tier):
    with connectDatabase() as connection:
        with connection.cursor() as cursor:
            sql = "UPDATE `whitelist_order` SET `tier` = %s WHERE `OrderID` = %s"
