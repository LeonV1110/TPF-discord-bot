...
def checkDuplicateUser(steam64ID, discordID):
    if db.checkSteamIDPressence(steam64ID):
        message = "This steam64ID is already in use"
        raise err.DuplicatePlayerPresent(steam64ID, discordID, message)
    elif db.checkDiscordIDPressence(discordID):
        message = "This discordID is already in use"
        raise err.DuplicatePlayerPresent(steam64ID, discordID, message)
    else:
        return
